...
static void Task_MainMenuCheckBattery(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        if (1) // was if (!(RtcGetErrorStatus() & RTC_ERR_FLAG_MASK))
        {
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                gTasks[taskId].func = Task_NewGameBirchSpeech_Init; // KURT: was Task_NewGameBirchSpeech_Init
                break;
...
static void Task_NewGameBirchSpeech_Init(u8 taskId)
{
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA, 0);
    SetGpuReg(REG_OFFSET_BLDY, 0);
    gSaveBlock2Ptr->playerGender=MALE; // KURT: added to initialize player gender
    NewGameBirchSpeech_SetDefaultPlayerName(Random() % NUM_PRESET_NAMES); // KURT: added to auto-select name
    ...
    gTasks[taskId].func = Task_NewGameBirchSpeech_FadePlayerToWhite; // KURT: was Task_NewGameBirchSpeech_WaitToShowBirch
    gTasks[taskId].tPlayerSpriteId = SPRITE_NONE;
    gTasks[taskId].data[3] = 0xFF;
    gTasks[taskId].tTimer = 0x0; // KURT: was 0xD8
